...
//				update_user_meta( $user_id, 'promo_code', $promo_code );
	...
	private function generate_promo_code( $user_id, $dob ) {
		...
		$promo_code = 'BDAY' . $user_id . $start_date->format( 'md' ) . $end_date->format( 'md' );
	...
	public static function get_users() {
		global $wpdb;
		$user_ids = $wpdb->get_col( "SELECT user_id FROM wp_usermeta WHERE MONTH(meta_value) = MONTH(DATE_ADD(NOW(), INTERVAL + 2 DAY)) AND DAY(meta_value) = DAY(DATE_ADD(NOW(), INTERVAL +2 DAY));" );
